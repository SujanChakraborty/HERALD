...
def _herald_adaptive_weights(h,
    ...
    """Verbatim from herald_condenser.py::_adaptive_weights(), with the
    sigmoid steepness/midpoint exposed as parameters for sensitivity sweeps
    (see ablation_study.py)."""
    t     = 1.0 / (1.0 + np.exp(-steepness * (h - midpoint)))
...
def condense_herald(data: Data, train: np.ndarray, splits: dict,
                    ...
                    feature_selector: str = "herald",
                    score_override: "t.Callable | None" = None,
                    weight_steepness: float = 8.0,
                    weight_midpoint: float = 0.4) -> dict:
    """
    ...
    Ablation hooks (all no-ops at their defaults, so default behaviour is
    byte-identical to the non-ablation code path):
      feature_selector : "herald" (default) uses the joint Fisher×density
                         selector; "bonsai" instead reuses Bonsai's WL+DT
                         feature indices (same k*, different indices),
                         isolating the effect of feature selection from
                         node scoring (see ablation_study.py, Table 2).
      score_override   : optional callable (score, train_idx) -> score,
                         applied to the raw combined score array right
                         before ranking. Used e.g. to inject a random
                         permutation for the "random ranking, same
                         pipeline" ablation control.
      weight_steepness, weight_midpoint : passed through to
                         _herald_adaptive_weights for sensitivity sweeps
                         over the sigmoid used to adapt (alpha,beta,gamma)
                         to the measured heterophily h. Ignored whenever
                         alpha/beta/gamma are given explicitly.
    """
    global _ADJ, _GLOBAL_NEIGHBORS_DICT, _GLOBAL_FEATS, _FEAT_LEN, _FEAT_MULTIPLIER
    SAINT = ["flickr", "ogbn-arxiv", "reddit"]
    t0 = time.perf_counter()
    ...
    if feature_selector == "herald":
        features_used = _herald_select_features(
            x_in, _ADJ, train_idx, data.y, h, top_k=n_select)
    elif feature_selector == "bonsai":
        # Ablation: same feature *count* (n_select), but keep Bonsai's own
        # WL+DT indices instead of HERALD's discriminativeness×density ones.
        # Isolates the contribution of feature selection from node scoring.
        features_used = sorted(bonsai_feats)
    else:
        raise ValueError(f"Unknown feature_selector '{feature_selector}'. "
                          f"Choose 'herald' or 'bonsai'.")

    if verbose:
        print(f"  [HERALD] {F} → {len(features_used)} features "
              f"(Bonsai DT count={n_select}, selector={feature_selector}, h={h:.2f})")
    ...
    if alpha is None or beta is None or gamma is None:
        ae, be, ge = _herald_adaptive_weights(
            h, steepness=weight_steepness, midpoint=weight_midpoint)
    else:
    ...
    # Ablation: e.g. random-ranking control — same feature selection, BFS
    # expansion, PPR pruning, and class rebalancing as full HERALD, but the
    # nodes are ranked randomly rather than by the combined score. Isolates
    # the contribution of node *scoring* from the contribution of the
    # downstream assembly pipeline (BFS+PPR+rebalance), which HERALD shares
    # with Bonsai.
    if score_override is not None:
        score = score_override(score, train_idx)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HERALD ABLATION VARIANTS
#
#  Each variant below is a thin wrapper around condense_herald that changes
#  exactly ONE axis (weighting scheme, which score terms are active, feature
#  selector, or node ranking) while leaving everything else — budget,
#  BFS/PPR/rebalance pipeline, dataset-dependent feat_len handling — byte
#  identical to the full method. This is what makes them valid ablations
#  rather than separate methods. See ablation_study.py for the driver script
#  that runs these against the compact (Cora / Amazon-ratings / Roman-empire)
#  matrix described in the paper's ablation section.
# ═══════════════════════════════════════════════════════════════════════════════

def condense_herald_fixed_weights(data, train, splits, scaler, dataset_name,
                                   dtype, target_size_frac, k=5,
                                   fixed=(0.4, 0.4, 0.2), **kw):
    """A1 — No adaptivity: use static (alpha0, beta0, gamma0) regardless of
    the graph's measured heterophily h, instead of letting them shift via
    the sigmoid in _herald_adaptive_weights."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            alpha=fixed[0], beta=fixed[1], gamma=fixed[2], **kw)


def condense_herald_proto_only(data, train, splits, scaler, dataset_name,
                                dtype, target_size_frac, k=5, **kw):
    """A2a — Prototype score only (alpha=1, beta=gamma=0)."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            alpha=1.0, beta=0.0, gamma=0.0, **kw)


def condense_herald_boundary_only(data, train, splits, scaler, dataset_name,
                                   dtype, target_size_frac, k=5, **kw):
    """A2b — Boundary score only (beta=1, alpha=gamma=0)."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            alpha=0.0, beta=1.0, gamma=0.0, **kw)


def condense_herald_lid_only(data, train, splits, scaler, dataset_name,
                              dtype, target_size_frac, k=5, **kw):
    """A2c — LID (diversity) score only (gamma=1, alpha=beta=0)."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            alpha=0.0, beta=0.0, gamma=1.0, **kw)


def condense_herald_no_lid(data, train, splits, scaler, dataset_name,
                            dtype, target_size_frac, k=5, **kw):
    """A2d — Prototype + boundary, no diversity term (alpha=beta=0.5, gamma=0)."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            alpha=0.5, beta=0.5, gamma=0.0, **kw)


def condense_herald_bonsai_feats(data, train, splits, scaler, dataset_name,
                                  dtype, target_size_frac, k=5, **kw):
    """A3 — Keep HERALD's adaptive node scoring, but swap in Bonsai's WL+DT
    feature indices (same count k*) instead of HERALD's discriminativeness
    x density selector. Isolates the contribution of feature selection."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            feature_selector="bonsai", **kw)


def _random_rank_override(seed):
    """Build a score_override callable that replaces the combined HERALD
    score with i.i.d. uniform noise on the training nodes only (so ranking
    among train_idx becomes a uniformly random permutation), leaving scores
    for non-training nodes untouched (they never enter `ranked` anyway)."""
    def _override(score, train_idx):
        rng = np.random.RandomState(seed)
        score = score.copy()
        score[train_idx] = rng.uniform(size=len(train_idx))
        return score
    return _override


def condense_herald_random_rank(data, train, splits, scaler, dataset_name,
                                 dtype, target_size_frac, k=5,
                                 seed=42, **kw):
    """A4 — Random-ranking control: identical feature selection, BFS
    expansion, PPR pruning, and class rebalancing as full HERALD, but
    training nodes are ranked uniformly at random instead of by the
    combined prototype/boundary/LID score. This is the key control that
    isolates the contribution of HERALD's node *scoring* from the
    contribution of the assembly *pipeline* it shares with Bonsai
    (BFS expand + PPR prune + rebalance) — mirrors Bonsai's own
    Random vs. Bonsai-Rev-k-NN ablation."""
    return condense_herald(data, train, splits, scaler, dataset_name, dtype,
                            target_size_frac, k=k,
                            score_override=_random_rank_override(seed), **kw)


HERALD_ABLATION_FN = {
    "herald_fixedw":       condense_herald_fixed_weights,
    "herald_proto_only":   condense_herald_proto_only,
    "herald_bound_only":   condense_herald_boundary_only,
    "herald_lid_only":     condense_herald_lid_only,
    "herald_no_lid":       condense_herald_no_lid,
    "herald_bonsai_feats": condense_herald_bonsai_feats,
    "herald_random_rank":  condense_herald_random_rank,
}
...
CONDENSER_FN.update(HERALD_ABLATION_FN)
