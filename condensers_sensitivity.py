...
def _herald_adaptive_weights(h,
    ...
    """Verbatim from herald_condenser.py::_adaptive_weights(), extended with
    tunable `steepness` and `midpoint` for the sigmoid transition variable t
    (Eq. 3 in the paper: t = sigmoid(steepness * (h - midpoint))).
    Defaults (steepness=8.0, midpoint=0.4) reproduce the paper exactly."""
...
def condense_herald(data: Data, train: np.ndarray, splits: dict,
                    ...
                    alpha0: float = 0.4, beta0: float = 0.4,
                    gamma0: float = 0.2,
                    steepness: float = 8.0, midpoint: float = 0.4) -> dict:
    """
    ...
    Tunable hyperparameters (all default to the exact values used in the
    paper, so passing none of them reproduces the main results exactly):
      lid_k              : k in Eq. (13), LID neighbourhood size.
      wl_layers          : L, BFS expansion depth (Eq. 15).
      alpha0, beta0, gamma0 : base weights (α0, β0, γ0) in Eq. (4).
      steepness, midpoint   : sigmoid steepness/midpoint in Eq. (3).
    """
    global _ADJ, _GLOBAL_NEIGHBORS_DICT, _GLOBAL_FEATS, _FEAT_LEN, _FEAT_MULTIPLIER
    SAINT = ["flickr", "ogbn-arxiv", "reddit"]
    t0 = time.perf_counter()
    ...
    if alpha is None or beta is None or gamma is None:
        ae, be, ge = _herald_adaptive_weights(
            h, alpha_base=alpha0, beta_base=beta0, gamma_base=gamma0,
